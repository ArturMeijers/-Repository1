...
while True:
    try:
        ...
        if field_size <= 0:
            ...
            print("Размер поля не может быть больше 11.")
        else:
            break
    except ValueError:
...
while True:
    try:
        ...
        elif num_ships > field_size * field_size:
...
for _ in range(num_ships):
    while True:
        length = random.randint(1, min(4, field_size))
        ...
        if is_vertical:
            ...
            # Проверяем, что все клетки для размещения корабля свободны
